...
if(targetPopulation.populationBasis === "Boolean"){
  ...
  if (
    ...
    const measurePopulation = Number(
      ...
      measurePopulationEx =targetPopulation.populationValues[2].name==="measurePopulationExclusion"?Number(
        targetPopulation.populationValues[2].expected):0
      ;
        const measPopLen = targetPopulation.populationValues[2].name==="measurePopulationExclusion"?4:3
    if (
      targetPopulation.populationValues.length <
      measPopLen + measurePopulation - measurePopulationEx
    ) {
      const measureObservationId = measureGroups.filter(
        (group) => group.id === changedGroupId
      )[0].measureObservations[0].id;
